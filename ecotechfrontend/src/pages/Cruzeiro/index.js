...
/**
 * Cruzeiro is a React component that renders environmental data for a specific station.
...
const Cruzeiro = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await fetch(
          `http://localhost:5000/api/cruzeiro?time_range=${timeRange}&interval=${interval}`
        );
        if (!response.ok) {
  ...
  return (
    <>
      <GlobalStyle />
      <Container>
        <StationTitle>Estação Cruzeiro</StationTitle>
        <DropdownContainer>
          <Dropdown
